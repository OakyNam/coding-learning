...
Learn files in Perl well enough to read examples, edit them, and use the idea in a small program.
...
In Perl, this topic shows up often at the intermediate level. Focus on the shape of the problem first: what data enters, what work happens, and what result should come out.

## Example

```
my @items = ("api", "json", "test");
for my $item (@items) {
    print uc($item), "\\n";
}
```
