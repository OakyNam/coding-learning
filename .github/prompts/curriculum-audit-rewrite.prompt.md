---
name: Curriculum Audit And Rewrite Workflow
description: "Use when: continuing the curriculum audit and rewrite process, coordinating director, audit, writer, review agents, TODO state, two-file rolling gate, validation, and platform compatibility checks."
agent: agent
...
Original source copy: `.codex/workflows/curriculum-audit-rewrite.md`
...
The specialist agent definitions live in the repo's `.github/agents/` folder as VS Code Copilot-recognized copies:

- `.github/agents/curriculum-director.agent.md` for macro curriculum strategy, learning-path planning, and `TODO.md` generation.
- `.github/agents/curriculum-assistant-director.agent.md` for read-only verification of director strategy and structure proposals.
- `.github/agents/curriculum-audit.agent.md` for read-only lesson or README audits.
- `.github/agents/curriculum-writer.agent.md` for single-file rewrites.
- `.github/agents/curriculum-review.agent.md` for read-only quality-gate reviews.

The original agent definitions remain copied in `.agents/`.
...
1. Dispatch `.github/agents/curriculum-director.agent.md` to assess scope, learner outcomes, learning paths, gap priorities, and the next execution clusters.
...
4. Dispatch `.github/agents/curriculum-assistant-director.agent.md` to review the director output for sequencing quality, repo evidence, path correctness, hallucination risk, and churn.
...
3. Dispatch specialist agents from `.github/agents/` to move each active file through audit, audit review, rewrite, rewrite review, validation, and completion.
...
1. `needs-audit` -> `audit-dispatched`: supervisor dispatches one read-only audit agent using `.github/agents/curriculum-audit.agent.md`; that agent researches and scopes exactly this file.
2. `audit-dispatched` -> `audit-reviewed`: supervisor dispatches a separate read-only review agent using `.github/agents/curriculum-review.agent.md`; that agent checks that the audit notes are specific, source-backed, implementation-ready, and scoped to the assigned file.
3. `audit-reviewed` -> `rewrite-dispatched`: supervisor dispatches one writer agent using `.github/agents/curriculum-writer.agent.md`; that agent rewrites exactly this file from the reviewed audit notes.
4. `rewrite-dispatched` -> `rewrite-reviewed`: supervisor dispatches a separate read-only review agent using `.github/agents/curriculum-review.agent.md`; that agent checks the rewritten file against the reviewed audit notes and the workflow quality gate.
...
- The supervisor must never edit assigned curriculum content directly. Curriculum file changes belong to writer agents using `.github/agents/curriculum-writer.agent.md`.
...
- Agents must use the configs in `.github/agents/curriculum-director.agent.md`, `.github/agents/curriculum-assistant-director.agent.md`, `.github/agents/curriculum-audit.agent.md`, `.github/agents/curriculum-writer.agent.md`, and `.github/agents/curriculum-review.agent.md`.
