---
name: Curriculum Strategy Refresh Workflow
description: "Use when: refreshing the macro curriculum plan, updating strategy files, dispatching curriculum director and assistant director, and approving TODO priority changes before rewrite work continues."
agent: agent
...
Original source copy: `.codex/workflows/curriculum-strategy-refresh.md`
...
- `.github/agents/curriculum-director.agent.md` owns the planning pass and may update the target files above.
- `.github/agents/curriculum-assistant-director.agent.md` owns the read-only review pass and must verify the updated strategy before execution resumes.
...
Dispatch `.github/agents/curriculum-director.agent.md` with strict scope limited to the five target files above.
...
After the director finishes, dispatch `.github/agents/curriculum-assistant-director.agent.md` against the same five files and any explicit move plan.
...
3. Resume or start the file-level workflow in `.github/prompts/curriculum-audit-rewrite.prompt.md`.
